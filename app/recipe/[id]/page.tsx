...
// ---------------------------------------------------------------------------
// Tag config (mirrors RecipeCard)
// ---------------------------------------------------------------------------
const TAG_CONFIG = {
  common:  { label: "Everyday",       emoji: "🥄", bg: "bg-sky-100",    text: "text-sky-700"    },
  good:    { label: "Impressive",     emoji: "⭐",  bg: "bg-violet-100", text: "text-violet-700" },
  special: { label: "Chef's Special", emoji: "👨‍🍳", bg: "bg-amber-100",  text: "text-amber-700"  },
};
...
function decodeRecipeFromHash(): Recipe | null {
  try {
    const hash = window.location.hash; // e.g. #recipe=eyJ...
...
export default function RecipeSharePage() {
  ...
  // Build share URL with OG search params so the layout.tsx can generate rich metadata
  const buildShareUrl = (r: Recipe) => {
    const base = `${window.location.origin}/recipe/share`;
    const params = new URLSearchParams({
      title: r.title,
      tag: r.tag ?? "special",
      ...(r.nutrition?.calories  ? { calories: r.nutrition.calories }  : {}),
      ...(r.nutrition?.prepTime  ? { prepTime: r.nutrition.prepTime }  : {}),
    });
      ...
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-950 px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">
            This share link is invalid or has been corrupted. The recipe data could not be decoded.
          </p>
          <Link
            href="/"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-brand-500 to-amber-500 text-white font-semibold px-6 py-3 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all"
          >
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300">
      {/* ---- Navbar ---- */}
      <nav className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-orange-100 dark:border-gray-700 shadow-sm">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition font-semibold text-sm">
            <ArrowLeftIcon />
            <span>Back to Pantry to Plate</span>
          </Link>
          <div className="flex items-center gap-2 text-gray-400 dark:text-gray-500 text-xs">
            <div className="w-6 h-6 rounded-md bg-gradient-to-br from-brand-500 to-amber-400 flex items-center justify-center">
              <ChefHatIcon />
            </div>
            <span className="hidden sm:inline font-semibold text-gray-600 dark:text-gray-300">Pantry to Plate</span>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mb-8 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-3 no-print">
            <button
              ...
              disabled={exporting}
              className={`flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-full transition-all duration-200 shadow-md
                ${exporting
            ...
            <button
              ...
              className={`flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-full border transition-all duration-200
                ${copied
                  ...
                  : "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-brand-300 dark:hover:border-brand-600 hover:scale-105"
                }`}
            >
              <ShareIcon />
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-full border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:border-brand-300 dark:hover:border-brand-600 hover:scale-105 transition-all duration-200"
            >
              <PrintIcon />
              Print recipe
            </button>
        ...
        {recipe!.nutrition && (
          <section className="mb-8 animate-slide-up" style={{ animationDelay: "60ms", animationFillMode: "both" }}>
            <h2 className="text-xs font-bold uppercase tracking-widest text-brand-500 dark:text-brand-400 mb-4">
        ...
        {/* ---- Footer CTA ---- */}
        <div className="text-center pt-6 border-t border-gray-100 dark:border-gray-800 animate-fade-in">
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">
            Want recipes from your own fridge?
          </p>
          <Link
            href="/"
      ...
      {/* ---- Footer ---- */}
      <footer className="mt-16 py-6 border-t border-gray-100 dark:border-gray-800 text-center text-xs text-gray-400 dark:text-gray-600">
