...
// ---------------------------------------------------------------------------
export default function RecipeShareClient() {
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-orange-100 dark:border-gray-700 shadow-sm no-print">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500 hidden sm:block font-medium">🍽️ Pantry to Plate</span>
        </div>
      </nav>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="flex flex-wrap gap-3 no-print">
            ...
            <button onClick={handleCopy}
              className={`flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-full border transition-all duration-200
                ${copied ? "bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800"
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {recipe!.ingredients.map((ing, i) => (
              <li key={i} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 rounded-xl px-4 py-3">
                <span className="w-5 h-5 rounded-full bg-brand-500 flex items-center justify-center flex-shrink-0 text-white"><CheckIcon /></span>
          ...
          <ol className="space-y-4">
            {recipe!.steps.map((step, i) => (
              <li key={i} className="flex gap-4">
                <span className="flex-shrink-0 w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900/50 text-brand-700 dark:text-brand-300 flex items-center justify-center text-sm font-bold">{i + 1}</span>
        ...
        <div className="text-center pt-6 border-t border-gray-100 dark:border-gray-800 no-print">
          ...
          <Link href="/" className="inline-flex items-center gap-2 bg-gradient-to-r from-brand-500 to-amber-500 text-white font-bold px-7 py-3 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">
            📷 Try Pantry to Plate
          </Link>
        </div>
      </main>
